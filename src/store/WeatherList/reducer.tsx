    ...
    switch (action.type) {
        case "LOAD LIST":
            return {
                ...state,
                load: true,
                list: Object.entries(state.self).length ? [state.self, ...(action.data.locations.length === 5 ? action.data.locations.slice(0, -1) : action.data.locations)] : action.data.locations,
        ...
        case "ADD LIST SELF":
            return {
                ...state,
                load: true,
                list: [action.data, ...(state.list.length === 5 ? state.list.slice(0, -1) : state.list)],
                self: action.data
            };
